...
public class StepChain : MonoBehaviour
{
	...
	/// <summary> Si esta o no pausada la ejecuccion de la cadena de Steps. </summary>
	/// Si esta en FALSE no se iniciara el siguiente paso al acabar el actual.
	bool paused = true;
	...
	public void StepEnded(Step step)
	{
		...
		if (!ValidIndex(currentStep) || stepList[currentStep] != step)
			return;
		...
		do {			
			currentStep++;
			RedrawHierarchy();
		} while (ValidIndex(currentStep) && stepList[currentStep] == null);

		// Activar el siguiente paso si no estamos en pausa o hemos llegado al final de la cadena.
		if (paused || !ValidIndex(currentStep))
			return;
	...
	/// <summary>
	/// Pausa la ejecuccion de la cadena de pasos. </summary>
	/// Esto no modifica ni paraliza el Step actual. Simplemente impide que se active el siguiente cuando acabe.
	public void Pause()
	{
		paused = true;
	}
	...
	void OnDrawHierarchyItem(int instanceId, Rect selectionRect)
	{
		...
		if (hierarchyItem == null || !ValidIndex(currentStep))
			return;
